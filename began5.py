...
if __name__ == '__main__':
    ...
    '''
    transforms = transforms.Compose([transforms.Resize(opt.fineSize),
                                     transforms.CenterCrop(opt.fineSize),
                                     transforms.ToTensor(),
                                     transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
                                     ])
    dataset = dset.ImageFolder('Img', transform=transforms)
    '''
    ...
    for iteration in range(1,opt.niter+1):
        ...
        # generate fake data
        with torch.no_grad():
            noise.resize_(images.size(0), opt.nz)
        ...
        balance = (opt.gamma * err_real - err_fake).item()
